...
import { BillingConfig, BillingWebhookHandlerService } from '@aihio/billing/core';
import { getLogger } from '@aihio/shared/logger';
import { Database, Enums } from '@aihio/supabase/database';
  ...
  async verifyWebhookSignature(request: Request) {
    ...
    if (!signature) {
      logger.error(
        ...
        `Signature header ei löytynyt`,
      );

      throw new Error('Signature header ei löytynyt');
    }
    ...
    if (!isValid) {
      logger.error(
        ...
        `Salainen avain on virheellinen`,
      );

      throw new Error('Salainen avain on virheellinen');
    }
