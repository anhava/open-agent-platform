...
import { Database, Json } from '@aihio/supabase/database';
...
class OtpService {
  ...
  async createNonce(params: CreateNonceParams) {
    ...
    logger.info(ctx, 'Luodaan kertakäyttöinen tokeni');
    ...
    try {
      ...
      if (result.error) {
        logger.error(
          ...
          'VIRHE: Kertakäyttöisen tokenin luominen epäonnistui',
        );

        throw new Error(
          `VIRHE: Kertakäyttöisen tokenin luominen epäonnistui: ${result.error.message}`,
        );
      }
      ...
      logger.info(
        ...
        'Kertakäyttöinen tokeni luotu onnistuneesti',
      );
      ...
      logger.error({ ...ctx, error }, 'VIRHE: Kertakäyttöisen tokenin luominen epäonnistui');
      throw error;
    }
  }
  ...
  async verifyNonce(params: VerifyNonceParams) {
    ...
    logger.info(ctx, 'Vahvistetaan kertakäyttöinen tokeni');
    ...
    try {
      ...
      if (result.error) {
        logger.error(
          { ...ctx, error: result.error.message },
          'VIRHE: Kertakäyttöisen tokenin vahvistaminen epäonnistui',
        );

        throw new Error(
          `VIRHE: Kertakäyttöisen tokenin vahvistaminen epäonnistui: ${result.error.message}`,
        );
      }
      ...
      logger.info(
        ...
        'Kertakäyttöisen tokenin vahvistaminen onnistui',
      );

      return data;
    } catch (error) {
      logger.error({ ...ctx, error }, 'VIRHE: Kertakäyttöisen tokenin vahvistaminen epäonnistui');
      throw error;
    }
  }
  ...
  async revokeNonce(params: RevokeNonceParams) {
    ...
    logger.info(ctx, 'Poistetaan kertakäyttöinen tokeni');
    ...
    try {
      ...
      if (error) {
        logger.error(
          ...
          'VIRHE: Kertakäyttöisen tokenin poistaminen epäonnistui',
        );

        throw new Error(`VIRHE: Kertakäyttöisen tokenin poistaminen epäonnistui: ${error.message}`);
      }

      logger.info(
        { ...ctx, success: data },
        'Kertakäyttöisen tokenin poistaminen onnistui',
      );
      ...
      logger.error({ ...ctx, error }, 'VIRHE: Kertakäyttöisen tokenin poistaminen epäonnistui');
      throw error;
    }
  }
  ...
  async getNonceStatus(params: GetNonceStatusParams) {
    ...
    logger.info(ctx, 'Hae kertakäyttöisen tokenin tila');
    ...
    try {
      ...
      if (result.error) {
        logger.error(
          { ...ctx, error: result.error.message },
          'VIRHE: Kertakäyttöisen tokenin tilaa ei saatu',
        );

        throw new Error(
          `VIRHE: Kertakäyttöisen tokenin tilaa ei saatu: ${result.error.message}`,
        );
      }
      ...
      logger.info(
        ...
        'Kertakäyttöisen tokenin tila saatu',
      );
      ...
      logger.error({ ...ctx, error }, 'VIRHE: Kertakäyttöisen tokenin tilaa ei saatu');
      throw error;
    }
  }
}
